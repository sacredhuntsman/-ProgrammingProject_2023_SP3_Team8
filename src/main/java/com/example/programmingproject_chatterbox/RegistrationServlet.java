...
public class RegistrationServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (dateOfBirth != null && isAtLeast13YearsOld(dateOfBirth)) {
	...
	private boolean isAtLeast13YearsOld(String dateOfBirth) {
		try {
			SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd");
			Date dob = dateFormat.parse(dateOfBirth);
			Date thirteenYearsAgo = new Date(System.currentTimeMillis() - 13L * 365 * 24 * 60 * 60 * 1000);
			
			return dob.before(thirteenYearsAgo);
